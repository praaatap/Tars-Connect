...
export function ChatRightSidebar({
    ...
    return (
        <aside className="flex h-full w-[280px] flex-col border-l border-zinc-200 bg-white">
            <div className="flex items-center justify-between border-b border-zinc-200 px-6 py-4 min-h-[73px] bg-zinc-50/50">
                <h2 className="text-xs font-black text-zinc-400 uppercase tracking-widest">Suggested For You</h2>
            </div>
                    ...
                    suggestedUsers.map((user: any) => (
                        <div
                            ...
                            className={`flex items-center rounded-xl px-2 py-2 text-left transition-all duration-200 group ${user.isInConversation
                                ? "hover:bg-indigo-50 cursor-pointer active:scale-[0.98]"
                                : user.hasPendingInvite
                                    ? "opacity-80 cursor-default"
                                    : "hover:bg-zinc-100 cursor-pointer active:scale-[0.98]"
                                }`}
                        >
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                                <div className="relative shrink-0 transition-transform group-hover:scale-105">
                                    {user.imageUrl ? (
                                        <img src={user.imageUrl} alt={user.name || "User"} className="h-8 w-8 rounded-full bg-zinc-200 object-cover ring-2 ring-transparent group-hover:ring-indigo-500/30" />
                                    ) : (
                                        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-100 text-xs font-bold text-indigo-700 ring-2 ring-transparent group-hover:ring-indigo-500/30">
                            ...
                            {user.isInConversation ? (
                                <div className="shrink-0 px-2 py-1 rounded-lg bg-emerald-100 flex items-center gap-1">
                                ...
                                <button
                                    ...
                                    className="shrink-0 px-3 py-1.5 rounded-lg bg-indigo-600 text-white text-[10px] font-bold hover:bg-indigo-700 transition-all cursor-pointer active:scale-90 shadow-sm opacity-0 group-hover:opacity-100 translate-x-1 group-hover:translate-x-0"
