    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 dark:bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-sm bg-white dark:bg-zinc-850 rounded-2xl shadow-2xl flex flex-col overflow-hidden max-h-[80vh]">
        ...
        <div className="p-6 space-y-4 overflow-y-auto flex-1">
          {/* Invite Link Section */}
          <div className="p-4 bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-200 dark:border-indigo-800 rounded-xl">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <svg className="w-5 h-5 text-indigo-600 dark:text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
                </svg>
                <label className="text-xs font-bold text-indigo-900 dark:text-indigo-100 uppercase">Invite Link</label>
              </div>
              {inviteToken && (
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-xs font-medium text-indigo-700 dark:text-indigo-300">Enabled</span>
                </label>
              )}
            </div>
            ...
            {!inviteToken ? (
              <button
                ...
                className="w-full py-2 px-4 bg-indigo-600 dark:bg-indigo-600 text-white rounded-lg text-sm font-semibold hover:bg-indigo-700 dark:hover:bg-indigo-700 transition-colors"
              >
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    readOnly
                    value={inviteLink || ''}
                    className="flex-1 px-3 py-2 bg-white dark:bg-zinc-900 border border-indigo-300 dark:border-indigo-700 rounded-lg text-xs text-zinc-700 dark:text-zinc-300 font-mono"
                  />
                  <button
                    onClick={handleCopyLink}
                    className="px-4 py-2 bg-indigo-600 dark:bg-indigo-600 text-white rounded-lg text-xs font-semibold hover:bg-indigo-700 dark:hover:bg-indigo-700 transition-colors"
                  >
                    {copied ? '✓ Copied' : 'Copy'}
                  </button>
                </div>
